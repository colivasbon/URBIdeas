  ...
  useEffect(() => {
    ...
    const geoJsonLayer = L.geoJSON(geojson, {
      style: (feature) => {
        ...
        const color = CLASE_SUELO_COLORS[clase] || '#95a5a6'
        return {
          color: '#333',
          weight: 0.5,
          opacity: 0.6,
        ...
        const popupContent = `
          <div style="font-family:var(--font-family);min-width:180px">
            <div style="display:flex;align-items:center;gap:6px;margin-bottom:6px">
              <div style="width:12px;height:12px;border-radius:3px;background:${color};border:1px solid #333"></div>
              <strong style="font-size:12px">${clase}</strong>
            </div>
            <table style="font-size:11px;border-collapse:collapse;width:100%">
              ${props.NuclRural ? `<tr><td style="padding:2px 6px 2px 0;color:#888">Núcleo rural</td><td style="padding:2px 0">${props.NuclRural}</td></tr>` : ''}
              ${props.AreaLamber ? `<tr><td style="padding:2px 6px 2px 0;color:#888">Área (Lambert)</td><td style="padding:2px 0">${Number(props.AreaLamber).toLocaleString()} m²</td></tr>` : ''}
            </table>
          </div>
        `
