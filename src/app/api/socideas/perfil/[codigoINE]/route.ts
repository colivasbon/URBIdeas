...
function pickLatest(values: IndicatorValue[], slug: string, extra?: (v: IndicatorValue) => boolean): IndicatorValue | null {
...
export async function GET(
  ...
  const codigoIne = (codigoINE ?? '').trim()
  ...
  try {
    ...
    if (valoresError) {
      ...
      if (code === 'PGRST205' || code === '42P01') {
        return NextResponse.json({ data: perfilSinDatos(), error: null, count: 0 })
      }
      throw valoresError
    }
    ...
    return NextResponse.json({ data: perfil, error: null, count: values.length })
  } catch (error) {
