      ...
      <div className="relative bg-[var(--color-card-bg-solid)] border border-[var(--color-border)] rounded-[var(--border-radius-xl)] w-full max-w-lg mx-4 max-h-[85vh] overflow-y-auto shadow-[var(--shadow-xl)] animate-slide-in-down">
        {title && (
          <div className="flex items-center justify-between border-b border-[var(--color-border-subtle)] px-6 py-4 sticky top-0 bg-[var(--color-card-bg-solid)] z-10">
            ...
            <button
              onClick={onClose}
              className="flex items-center justify-center w-8 h-8 rounded-full text-[var(--color-text-muted)] hover:text-[var(--color-text-primary)] hover:bg-[var(--color-input-bg)] transition-colors duration-[var(--duration-fast)]"
              aria-label="Cerrar"
            >
