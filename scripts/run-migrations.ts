...
async function runMigration016() {
  console.log('\n=== MIGRACIÓN 016: Ampliar modelo normativa ===')
  ...
  // Expand ambito CHECK constraint
  await sql`ALTER TABLE normativa_vigente DROP CONSTRAINT IF EXISTS normativa_vigente_ambito_check`
  await sql`ALTER TABLE normativa_vigente ADD CONSTRAINT normativa_vigente_ambito_check CHECK (ambito IN ('estatal', 'autonomico', 'provincial', 'municipal', 'planeamiento'))`
  console.log('CHECK constraint de ambito ampliado.')
  ...
  console.log('Datos de trazabilidad actualizados en normas existentes.')
}
